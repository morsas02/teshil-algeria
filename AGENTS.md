...
- **النشر**: من مجلد المشروع:
  ...
  **مهم**: حتى مع `-w`، أضف `</dev/null` (إغلاق stdin) وإلا يعلق عند "Indexing..." بدون أي رسالة بعدها — هذا ما يحدث مع CLI 5.30.x.
  يمكن متابعة الحالة بـ `railway deployment list`). النشر عبر دفع GitHub لا يعمل بشكل موثوق.
