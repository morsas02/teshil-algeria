...
ADMIN_EMAIL = "admin@ta9eef.dz"
ADMIN_PASSWORD = "admin123456"
JOB_PRICE = 1000  # دج لكل نشر وظيفة (يمكن تغييره من الإعدادات)
...
def worker_required(f):
    ...
    def decorated(*args, **kwargs):
        if session.get('user_type') != 'worker':
            flash('هذه الصفحة مخصصة للعمال فقط', 'danger')
            return redirect(url_for('index'))
        return f(*args, **kwargs)
    return decorated
...
def get_unread_count(user_id):
    conn = get_db()
    count = conn.execute('SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND is_read = 0', (user_id,)).fetchone()
    conn.close()
    return count['c']
...
def inject_globals():
    ...
    if 'user_id' in session:
        ...
        if session.get('user_type') == 'employer':
            ...
            ctx['wallet_balance'] = user['wallet_balance'] if user else 0
            price = conn.execute('SELECT value FROM settings WHERE key = ?', ('job_price',)).fetchone()
...
def register():
    if request.method == 'POST':
        ...
        if user_type == 'worker':
            conn.execute('INSERT INTO workers (user_id) VALUES (?)', (user_id,))
        else:
            company_name = request.form.get('company_name', '').strip() or full_name
            conn.execute('UPDATE users SET wallet_balance = ? WHERE id = ?', (0, user_id))
            conn.execute('INSERT INTO employers (user_id, company_name) VALUES (?, ?)', (user_id, company_name))
...
def google_callback():
    ...
    if user:
        ...
        import re
        cur = conn.execute(
            'INSERT INTO users (full_name, email, password, user_type, is_verified) VALUES (?, ?, ?, ?, ?)',
...
def forgot_password():
    if request.method == 'POST':
        email = request.form['email'].strip().lower()
        conn = get_db()
        user = conn.execute('SELECT id, full_name FROM users WHERE email = ?', (email,)).fetchone()
        if user:
            ...
            flash(f'رابط إعادة تعيين كلمة المرور: {reset_link}', 'info')
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        conn.execute('UPDATE reset_tokens SET used = 1 WHERE id = ?', (record['id'],))
        conn.commit()
        conn.close()
...
def profile():
    ...
    if session['user_type'] == 'worker':
        user = conn.execute('''
            ...
            JOIN workers w ON w.user_id = u.id WHERE u.id = ?
        ''', (session['user_id'],)).fetchone()

        if request.method == 'POST':
            conn.execute('''
                UPDATE workers SET skills=?, experience_years=?, experience_level=?,
                education=?, city=?, wilaya=?, about=?, availability=?,
                expected_salary=?, linkedin_url=?, portfolio_url=?
                WHERE user_id=?
            ''', (
        ...
        user = conn.execute('''
            ...
            JOIN employers e ON e.user_id = u.id WHERE u.id = ?
        ''', (session['user_id'],)).fetchone()

        if request.method == 'POST':
            conn.execute('''
                UPDATE employers SET company_name=?, company_description=?,
                company_website=?, company_size=?, company_sector=?,
                city=?, wilaya=?, address=?
                WHERE user_id=?
            ''', (
...
def jobs():
    conn = get_db()
    query = '''
        SELECT j.*, e.company_name, e.company_logo, u.full_name
        FROM jobs j JOIN employers e ON j.employer_id = e.id
        JOIN users u ON e.user_id = u.id
        WHERE j.status = 'approved'
    '''
    ...
    return render_template('jobs.html', jobs=jobs, search=search, wilaya=wilaya,
                         category=category, contract=contract, experience=experience, sort=sort)
...
def job_detail(job_id):
    ...
    conn.execute('UPDATE jobs SET views_count = views_count + 1 WHERE id = ?', (job_id,))
    ...
    if 'user_id' in session and session['user_type'] == 'worker':
        ...
        if worker:
            ...
            is_saved = bool(conn.execute(
                'SELECT id FROM saved_jobs WHERE job_id = ? AND worker_id = ?',
                (job_id, worker['id'])
            ).fetchone())
...
def create_job():
    if request.method == 'POST':
        conn = get_db()
        user = conn.execute('SELECT wallet_balance FROM users WHERE id = ?', (session['user_id'],)).fetchone()
        price_row = conn.execute('SELECT value FROM settings WHERE key = ?', ('job_price',)).fetchone()
        ...
        employer = conn.execute('SELECT id FROM employers WHERE user_id = ?', (session['user_id'],)).fetchone()
        ...
        conn.execute('''
            ...
            VALUES (?, 'debit', ?, ?, ?, ?, 'job_post', 'completed')
...
def toggle_job(job_id):
    conn = get_db()
    employer = conn.execute('SELECT id FROM employers WHERE user_id = ?', (session['user_id'],)).fetchone()
    job = conn.execute('SELECT status FROM jobs WHERE id = ? AND employer_id = ?', (job_id, employer['id'])).fetchone()
...
def delete_job(job_id):
    conn = get_db()
    employer = conn.execute('SELECT id FROM employers WHERE user_id = ?', (session['user_id'],)).fetchone()
    conn.execute('DELETE FROM jobs WHERE id = ? AND employer_id = ?', (job_id, employer['id']))
    conn.commit()
    conn.close()
...
def apply_job(job_id):
    conn = get_db()
    worker = conn.execute('SELECT id FROM workers WHERE user_id = ?', (session['user_id'],)).fetchone()
    existing = conn.execute('SELECT id FROM applications WHERE job_id = ? AND worker_id = ?', (job_id, worker['id'])).fetchone()
    if existing:
        flash('لقد تقدمت لهذه الوظيفة مسبقاً', 'warning')
    else:
        conn.execute('INSERT INTO applications (job_id, worker_id, cover_letter) VALUES (?, ?, ?)',
                    (job_id, worker['id'], request.form.get('cover_letter', '')))
        conn.execute('UPDATE jobs SET applications_count = applications_count + 1 WHERE id = ?', (job_id,))

        job = conn.execute('SELECT employer_id, title FROM jobs WHERE id = ?', (job_id,)).fetchone()
        employer_user = conn.execute('SELECT user_id FROM employers WHERE id = ?', (job['employer_id'],)).fetchone()
...
def save_job(job_id):
    conn = get_db()
    worker = conn.execute('SELECT id FROM workers WHERE user_id = ?', (session['user_id'],)).fetchone()
    existing = conn.execute('SELECT id FROM saved_jobs WHERE job_id = ? AND worker_id = ?', (job_id, worker['id'])).fetchone()
    if existing:
        conn.execute('DELETE FROM saved_jobs WHERE id = ?', (existing['id'],))
        flash('تم إزالة الوظيفة من المحفوظات', 'info')
    else:
        conn.execute('INSERT INTO saved_jobs (job_id, worker_id) VALUES (?, ?)', (job_id, worker['id']))
...
def employer_applications():
    ...
    jobs_list = conn.execute('SELECT id, title FROM jobs WHERE employer_id = ?', (employer['id'],)).fetchall()
    conn.close()
...
def handle_application(app_id, action):
    ...
    if app and employer:
        job = conn.execute('SELECT employer_id FROM jobs WHERE id = ?', (app['job_id'],)).fetchone()
        if job['employer_id'] == employer['id']:
            conn.execute('UPDATE applications SET status = ? WHERE id = ?', (action, app_id))
            worker = conn.execute('SELECT user_id FROM workers WHERE id = ?', (app['worker_id'],)).fetchone()
...
def buy_package():
    ...
    conn = get_db()
    package = conn.execute('SELECT * FROM packages WHERE id = ? AND is_active = 1', (package_id,)).fetchone()
    if not package:
    ...
    conn.execute('''
        ...
        VALUES (?, 'credit', ?, ?, ?, ?, 'package', ?, 'pending')
    ''', (
        session['user_id'], package['credits'], user['wallet_balance'],
        user['wallet_balance'] + package['credits'],
        f'شراء باقة: {package["name"]} ({package["credits"]} رصيد)', package['id']
    ))
    conn.commit()
    conn.close()
    flash(f'تم طلب شراء باقة "{package["name"]}". في انتظار تأكيد الدفع من الإدارة.', 'info')
    return redirect(url_for('employer_wallet'))
...
def employer_topup():
    ...
    ''', (
        session['user_id'], amount, user['wallet_balance'],
        user['wallet_balance'] + amount, f'شحن رصيد بقيمة {amount} دج'
    ))
    conn.commit()
    conn.close()
    flash(f'تم طلب شحن {amount} دج. في انتظار تأكيد الدفع من الإدارة.', 'info')
    return redirect(url_for('employer_wallet'))
...
def my_applications():
    ...
    apps = conn.execute('''
        SELECT a.*, j.title as job_title, j.wilaya, j.city, j.contract_type, j.salary_min, j.salary_max,
               e.company_name, u.full_name, u.phone, u.email
        FROM applications a
        JOIN jobs j ON a.job_id = j.id
        JOIN employers e ON j.employer_id = e.id
        JOIN users u ON e.user_id = u.id
...
def saved_jobs():
    ...
    jobs = conn.execute('''
        ...
        JOIN employers e ON j.employer_id = e.id
        JOIN users u ON e.user_id = u.id
        WHERE sj.worker_id = ? AND j.status = 'approved'
...
def admin_toggle_verify(user_id):
    ...
    if user:
        ...
        conn.execute('UPDATE users SET is_verified = ? WHERE id = ?', (new, user_id))
        conn.commit()
...
def admin_handle_job(job_id, action):
    ...
    conn = get_db()
    if action == 'feature':
        job = conn.execute('SELECT is_featured FROM jobs WHERE id = ?', (job_id,)).fetchone()
        if job:
            conn.execute('UPDATE jobs SET is_featured = ? WHERE id = ?', (0 if job['is_featured'] else 1, job_id))
    elif action == 'approve':
        conn.execute('UPDATE jobs SET status = "approved" WHERE id = ?', (job_id,))
        job = conn.execute('SELECT employer_id, title FROM jobs WHERE id = ?', (job_id,)).fetchone()
        employer_user = conn.execute('SELECT user_id FROM employers WHERE id = ?', (job['employer_id'],)).fetchone()
        ...
        job = conn.execute('SELECT employer_id, title FROM jobs WHERE id = ?', (job_id,)).fetchone()
        employer_user = conn.execute('SELECT user_id FROM employers WHERE id = ?', (job['employer_id'],)).fetchone()
        notify(employer_user['user_id'], 'لم يتم الموافقة على وظيفتك',
               f'نأسف، لم تتم الموافقة على وظيفة "{job["title"]}". يرجى مراجعة شروط النشر.', 'danger', '/my-jobs')
...
def admin_handle_transaction(tid, action):
    ...
    if action == 'confirm':
        ...
        conn.execute("UPDATE transactions SET status = 'cancelled' WHERE id = ?", (tid,))
...
def admin_adjust_wallet(user_id):
    ...
    conn.execute('UPDATE users SET wallet_balance = ? WHERE id = ?', (new_balance, user_id))
    conn.execute('''
        INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, description, reference_type, status)
...
def admin_settings():
    ...
    if request.method == 'POST':
        ...
        if job_price and job_price > 0:
            conn.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('job_price', str(job_price)))
        for key in ['site_name', 'site_description', 'contact_email']:
...
def admin_add_package():
    conn = get_db()
    conn.execute('INSERT INTO packages (name, credits, price, duration_days) VALUES (?, ?, ?, ?)', (
...
def admin_toggle_package(pid):
    conn = get_db()
    pkg = conn.execute('SELECT is_active FROM packages WHERE id = ?', (pid,)).fetchone()
    if pkg:
        conn.execute('UPDATE packages SET is_active = ? WHERE id = ?', (0 if pkg['is_active'] else 1, pid))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_settings'))
